...
public interface ParcelleRepository extends IAbstractRepository<Parcelle> {
    ...
    @Query(value = "SELECT * FROM kelrisks.cadastre AS p" +
                   " WHERE st_dwithin(:point, p.geog, (100. / 100000.))" +
                   " ORDER BY st_distance(p.geog, :point)" +
                   " LIMIT 1 ", nativeQuery = true)
    Parcelle rechercherClosestParcelleAvecPoint(Geometry<?> point);
    
    @Query(value = "SELECT * FROM kelrisks.cadastre AS p" +
                   " WHERE st_touches(p.geog, :geog)", nativeQuery = true)
    List<Parcelle> rechercherParcellesContigues(Geometry<?> geog);
    
    @Query(value = "SELECT * FROM kelrisks.cadastre AS p" +
                   " WHERE st_dwithin(st_setsrid(st_point(:x, :y),4326), p.geog, (100. / 100000.))" +
                   " ORDER BY st_distance(p.geog, st_setsrid(st_point(:x, :y),4326))" +
                   " LIMIT 1 ", nativeQuery = true)
    Parcelle rechercherClosestParcelleAvecCoordonnees(double x, double y);
    
    @Query(value = "SELECT st_asewkt(st_buffer(:parcelleGeog, :distance))", nativeQuery = true)
    String rechercherExpendedParcelle(Geometry<?> parcelleGeog, double distance);
    
    @Query(value = "SELECT st_asewkt(st_union(p.geog)) FROM kelrisks.cadastre AS p" +
                   " WHERE st_touches(p.geog, :geog)", nativeQuery = true)
    String rechercherUnionParcellesContigues(Geometry<?> geog);
    
    @Query(value = "SELECT st_asewkt(st_centroid(:polygon))", nativeQuery = true)
    String rechercherCentroidParcelle(Geometry<?> polygon);
    
    @Query(value = "SELECT st_asewkt(st_intersects(:polygon, p.geog)) " +
    ...
    @Query(value = "SELECT * " +
                   ...
                   " WHERE st_dwithin(st_setsrid(st_point(:x ,:y), 4326), p.geog, :radius)", nativeQuery = true)
