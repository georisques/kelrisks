...
export default {
    ...
    data: () =>({
        ...
        // https://github.com/tsayen/dom-to-image/pull/358
    ...
    watch: {
        ...
        mapCentered: function () {
            if (this.mapCentered) {
                ...
                // FIXME pas trouver une methode plus propre pour ajouter l'attribution sur cette couche geojson
                if(this.pngName === 'POLLUTION_NON_REG'){
                    map.attributionControl.addAttribution(conf.config.attributions.POLLUTION_NON_REG)
                } else {
                    map.attributionControl.removeAttribution(conf.config.attributions.POLLUTION_NON_REG)
                }

                console.log("hasWmsLayer",hasWmsLayer, this.pngName)
